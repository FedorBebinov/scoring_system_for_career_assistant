...
def set_vacancy_by_career_objective(user, jobs):
    ...
    for name, id in jobs[["НазваниеПреобр", "ID"]].values:
...
def load_data(experience, vacancy):
    vacancies = pd.read_excel("data/jobs_edited.xlsx")
    pymorphy2_analyzer_ru = MorphAnalyzer()
    analyzer_en = WordNetLemmatizer()
    with open('stopwords-ru.json', 'r') as openfile:
    ...
    vacancies['Детальное описание'] = vacancies['Детальное описание'].replace('_x000d_', ' ', regex=True)
    vacancies['Детальное описание'] = vacancies['Детальное описание'].replace('_x000D_', ' ', regex=True)
    vacancies['Детальное описание'] = vacancies['Детальное описание'].replace("\n", ' ', regex=True)
    vacancies['Детальное описание'] = vacancies['Детальное описание'].replace('\[(.*?)\]', '', regex=True)
    vacancies['Детальное описание'] = vacancies['Детальное описание'].apply(lambda vacancy: ' '.join(vacancy.split()))
    vacancies['Детальное описание'] = vacancies.apply(
        lambda text: preprocess_text(text['Детальное описание'], stop_words, pymorphy2_analyzer_ru, analyzer_en),
        axis=1)
    vacancies['НазваниеПреобр'] = vacancies['Название'].apply(
        lambda obj: preprocess_text(obj, stop_words, pymorphy2_analyzer_ru, analyzer_en))
...
def create_vectorizer(vacancies):
    ...
    overall_texts = vacancies['Детальное описание'].values.tolist()
    tf_idf_vec = TfidfVectorizer()
    tf_idf_vec.fit(overall_texts)
    X = tf_idf_vec.transform(vacancies['Детальное описание'])
    X.toarray()
...
def process_users(tf_idf_vec, users_texts, users, vacancies):
    ...
    vacancies_with_id = vacancies.set_index(['ID'])#для итерации
    users_texts['TF_IDF_VEC'] = users_df.apply(lambda user: user.to_numpy(), axis=1)#вектора приводим к нумпи массиву, чтобы проще рабоатть(вакансии уже переведены)
...
#vacancies, users_texts, users = load_data("закончил курс образование", "программист")
#vectorizer = create_vectorizer(vacancies)
#print(process_users(vectorizer, users_texts, users, vacancies))
